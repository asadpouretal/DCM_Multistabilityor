function [acc,decision_reaction_times,error_reaction_times,decision_trial_indicesa,error_trial_indicesa,non_decision_trial_indicesa,o1_e1,o1_e2,o1_i1,o2_e1,o2_e2,o2_i1,input_1,input_2,avg_decision_rts,avg_error_rts,output1_LFP,output2_LFP]=fsm3(num_trials,fs,decision_boundaries,a1)
    % Parameters
    Ae = 3.25; %H_e
    Ai = 22;   %H_i
    a1e = 10;  %K_e
    ...
    cLi21 = 19;  %excitatory-inhibitory lateral connection
    cLi12 = 19;  %excitatory-inhibitory lateral connection
...
% Guassian bump as input
    ...
    for trial = 1:num_trials
        ...
        X(1, :) = X0;
        for i = 1:length(t)-1
            X(i+1, :) = X(i, :) + dt * model3(X(i, :), i, bump1, bump2, noise_amp, tampa, Ae, Ai, a1e, a1i, cL21, cL12, cLi21, cLi12,c1,c2,c3,c4,c5,c6,c7); %Simulation of the model
        end
        ...
        % Classification of trials
            ...
            % Both increasing
            if max(x2) >= max(x17) && abs(max(abs(x2)) - max(abs(x17))) >= decision_boundaries
                decision_trial_indices = [decision_trial_indices, trial];
            elseif max(x17) >= max(x2) && abs(max(abs(x17)) - max(abs(x2))) >= decision_boundaries
...
non_decision_trial_indicesa=non_decision_trial_indices;
    ...
    for trial_idx = 1:length(decision_trial_indices)
        ...
        X(1, :) = X0;
        for i = 1:length(t)-1
            X(i+1, :) = X(i, :) + dt * model3(X(i, :), i, bump1, bump2, noise_amp, tampa, Ae, Ai, a1e, a1i, cL21, cL12, cLi21, cLi12, c1,c2,c3, c4, c5,c6, c7);
        end
        %x2 = X(:, 1);
        %X1=-(X(:, 2)'-X(:, 3)'+X(:, 7)');
        %X2=-(X(:, 16)'-X(:, 17)'+X(:, 21)');
