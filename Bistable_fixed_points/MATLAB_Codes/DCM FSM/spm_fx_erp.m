...
%   x(:,2) - voltage (pyramidal cells) +ve : e2
%   x(:,3) - voltage (pyramidal cells) -ve : e2
...
%--------------------------------------------------------------------------
E = [1 1/2 1/8]*32;         % extrinsic rates (forward, backward, lateral) %% OK
...
G = [0.1 0.1 15 3 3 0.1 30];        % intrinsic rates (g1 g2 g3 g4 g5 g6 g7) %% OK


D = [0 0];                 % propogation delays (intrinsic, extrinsic)
H = [3.25 22];                 % receptor densities (excitatory, inhibitory)    %%OK
T = [100 5];                 % synaptic constants (excitatory, inhibitory)      %% OK
R = [2 1]/3;                % parameters of static nonlinearity                 %% OK 
...
%--------------------------------------------------------------------------
f(:,1) = x(:,8);
f(:,8) = (He.*((A{1} + A{3})*S(:,17) + G(:,1).*(S(:,17)) + U)    - 2*x(:,8)  - x(:,1)./Te)./Te;     %% OK

% Infra-granular layer (pyramidal cells): depolarizing current
%--------------------------------------------------------------------------
f(:,2) = x(:,9);
f(:,9) = (He.*((A{2})*S(:,17) + G(:,2).*(S(:,15)))        - 2*x(:,9)  - x(:,2)./Te)./Te;            %% OK 

% Infra-granular layer (pyramidal cells): hyperpolarizing current
%--------------------------------------------------------------------------
f(:,3) = x(:,10);
f(:,10)= (Hi.*G(:,3).*(S(:,16))         - 2*x(:,10) - x(:,3)./Ti)./Ti;                              %% OK                          

% Supragranular layer (inhibitory interneurons): depolarizing current
%--------------------------------------------------------------------------
f(:,4) = x(:,11);
f(:,11)= (He.*((A{2} + 19/0.6 * A{3})*S(:,17) + G(:,4).*(S(:,17)) + U)- 2*x(:,11) - x(:,4)./Te)./Te; % modified included U  %% OK

% Supragranular layer (inhibitory interneurons): hyperpolarizing current
%--------------------------------------------------------------------------
f(:,5) = x(:,12);
f(:,12)= (Hi.*G(:,5).*(S(:,16))         - 2*x(:,12) - x(:,5)./Ti)./Ti;                              %% OK

% Granular layer (spiny stellate cells):feedback loop
%--------------------------------------------------------------------------
f(:,6) = x(:,13);
f(:,13)= (He.*G(:,6).*(S(:,15))        - 2*x(:,13) - x(:,6)./Te)./Te;                               %% OK

% Infra-granular layer (pyramidal cells): feedback loop
%--------------------------------------------------------------------------
f(:,7) = x(:,14);
f(:,14)= (He.*G(:,7).*(S(:,17)) - 2*x(:,14) - x(:,7)./Te)./Te;                                      %% OK

% Feedback connections
%--------------------------------------------------------------------------
f(:,15)= x(:,8) + x(:,13);          % dx1 + dx6
f(:,16)= x(:,11)- x(:,12);          % dx4 - dx5
% Output
f(:,17)= x(:,9) - x(:,10) + x(:,14);    % dx2 - dx3 + dx7
f(:,18)= x(:,1) + x(:, 6) + x(:, 2) + x(:, 7) - x(:, 3) + x(:, 4) - x(:, 5);    % LFP output
