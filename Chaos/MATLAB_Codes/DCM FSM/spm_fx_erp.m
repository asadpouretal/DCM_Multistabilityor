...
% state equations for a neural mass model of erps for chaos
...
inhibitory_to_excitatory_ratio = [32/74.8025; 144/74.8025];   % inhibitory to excitatory extrinsic connection ratio to each region [column 1 to column 2; column 2 to column 1] %OK
...
T = [0.02 0.01];                 % synaptic constants (excitatory, inhibitory) in seconds  % OK     
R = [1/5 30 1/2];                % parameters of static nonlinearity  (excitatory gm^-1, V*, inhibitory g^-1)       %OK         
...
%--------------------------------------------------------------------------
f(:,3) = -x(:, 3)./Ti + G(:, 4).* S(:, 2) + inhibitory_to_excitatory_ratio .* A{3} * S(:, 2) - G(:, 5).* S(:, 3) + A{2} * S(:, 1) + U; %(Hi.*G(:,3).*(S(:,16))         - 2*x(:,10) - x(:,3)./Ti)./Ti;                             %OK
