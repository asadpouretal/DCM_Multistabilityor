function performDCMBMSBatch(dataFolder, modelNames, outputFolder)
    ...
    for i = 1:length(trialKeys)
        if trialData.(trialKeys{i}).complete
            ...
            % row = struct('Trial', str2double(trialData.(trialKeys{i}).trial));
            % for j = 1:length(modelNames)
            %     dcm = load(trialData.(trialKeys{i}).files{j}, 'DCM');
            %     safeName = matlab.lang.makeValidName(['F_' modelNames{j}]);
            %     row.(safeName) = dcm.DCM.F;
            % end
            % logEvidenceTable = [logEvidenceTable; row];
        end
    end
    ...
    if nSessions > 0
        ...
        % Load BMS results
        bms_path = fullfile(outputFolder, 'BMS.mat');
        if exist(bms_path, 'file')
            load(bms_path, 'BMS');
        
            % Extract and plot log model evidence matrix
            F = BMS.DCM.rfx.F;  % [nTrials × nModels]
            
            figure;
            bar(log10(F), 'grouped');
            xlabel('Trial');
            ylabel('log_{10} Free Energy');
            % legend(modelNames, 'Location', 'northeastoutside');
            title('BMS.DCM.rfx.F: Log Model Evidence per Trial');
            xticks(1:size(F,1));
            grid off;
        else
            warning('BMS.mat not found. Skipping BMS.DCM.rfx.F plot.');
        end
    else
