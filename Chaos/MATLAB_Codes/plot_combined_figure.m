function plot_combined_figure()
    ...
    % Load BMS.mat files
    correct_trials_file = fullfile(correct_trials_dir, 'combined','BMS.mat');

    correct_trials_data = load(correct_trials_file);

    % Extract model names and exceedance probabilities
    model_names = correct_trials_data.BMS.DCM.rfx.family.names;
    ...
    correct = [correct_trials_data.BMS.DCM.rfx.bma.mEp.A{3}(1,2), correct_trials_data.BMS.DCM.rfx.bma.mEp.A{3}(2,1)];
    ...
    % Combine data for plotting
    data_lateral = [ground_truth; correct]';
